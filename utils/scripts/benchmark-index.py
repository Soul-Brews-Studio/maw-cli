...
ROOT = common.ROOT
spec = importlib.util.spec_from_file_location("index_smoke", ROOT / "utils/scripts/index-smoke.py")
...
def hashes(languages):
    ...
    for name in ["utils/scripts/benchmark-index.py", "utils/scripts/index-smoke.py", "docs/trace-index-contract.md", "utils/just/bench.just"]:
...
def main():
    ...
    output = (args.output or ROOT / "docs/benchmarks/cli/results" / f"index-{stamp}.json").absolute()
    ...
    result = dict(timestamp_utc=stamp, commit=common.capture([common.tool("git"), "rev-parse", "HEAD"]),
                  dirty=bool(common.capture([common.tool("git"), "status", "--porcelain", "--", *["src/" + language for language in languages], "utils/scripts", "docs/trace-index-contract.md", "utils/just/bench.just"])),
