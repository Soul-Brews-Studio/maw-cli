...
def commands(language, scratch=None):
    ...
    if language == "go":
        ...
        output = scratch / "dist" if scratch else ROOT / "src/js/dist"
        build = [tool("bun"), "build", str(ROOT / "src/js/src/cli.ts"), "--target=bun", "--outdir", str(output)]
...
def source_hashes(languages):
    files = ["utils/scripts/benchmark.py", "utils/scripts/smoke.sh", "go.work"]
    for language in languages:
        files.extend(footprint(language)["files"])
        files.extend(str(p.relative_to(ROOT)) for p in (ROOT / "src" / language).iterdir()
...
def main():
    ...
    output = (args.output or ROOT / "docs/benchmarks/cli/results" / f"{stamp}.json").absolute()
    result = {"timestamp_utc": stamp, "source_root": str(ROOT),
              "commit": capture([tool("git"), "rev-parse", "HEAD"]),
              "dirty": bool(capture([tool("git"), "status", "--porcelain", "--", *["src/" + language for language in languages],
                                     "go.work", "utils/scripts/benchmark.py", "utils/scripts/smoke.sh"])),
    ...
    with tempfile.TemporaryDirectory(prefix="maw-benchmark-") as temporary:
        ...
        for language in languages:
            ...
            smoke = [tool("sh"), str(ROOT / "utils/scripts/smoke.sh"), *run]
        ...
        for language, entry in result["languages"].items():
            ...
            if args.builds:
                ...
                capture([tool("sh"), str(ROOT / "utils/scripts/smoke.sh"), *run])
