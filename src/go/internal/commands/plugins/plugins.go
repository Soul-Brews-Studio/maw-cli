...
func (p *plugin) Metadata() command.Metadata {
	...
	return command.Metadata{Name: "plugin", Summary: "List, install, update and inspect plugins", Usage: "maw plugin ls|list [-v|--verbose] [--all] | install SOURCE[@REF|#REF] [--ref REF] | install FILE.tar.gz|HTTPS.tar.gz [--backup|--replace] | update NAME[@REF|#REF] [--ref REF] | info|check NAME"}
}
func (*plugin) BindFlags(*flag.FlagSet) {}
func (p *plugin) Run(ctx context.Context, i *command.Invocation) int {
	args := i.Args
	if len(args) > 0 && (args[0] == "install" || args[0] == "update" || args[0] == "info" || args[0] == "check") {
		return lifecycle(ctx, i)
	}
